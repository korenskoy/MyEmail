...
Review changed code for reuse, quality, efficiency, and clarity issues. Use Codex agents to review in parallel, but keep those agents read-only: they should only inspect code and send findings back to the main agent. Only the main agent may apply high-confidence, behavior-preserving fixes.
...
Use Codex agents when the scope is large enough for parallel review to help. For a tiny diff or one very small file, it is acceptable to review locally instead.

When spawning agents:

- give each agent the same scope
- tell each agent to inspect only its assigned review role
- tell each agent it is operating in a read-only review pass
- do not let agents use Edit, Write, or Bash tools to modify files, stage changes, commit, or perform other state-mutating actions
- ask for concise, structured findings only
- ask each agent to report file, line or symbol, problem, recommended fix, and confidence
- ask each agent to return findings to the main agent only; they must not implement fixes themselves
...
This agent is read-only. It must not edit files, apply patches, or make any other workspace changes.

Use the Agent tool with `subagent_type: "Explore"` for broad codebase lookup, or `subagent_type: "general-purpose"` for a deeper review pass.
...
Use the Agent tool with `subagent_type: "general-purpose"` for this review pass.
...
Wait for all review agents to complete, then merge their findings.

The main agent owns this step. Treat agent output as review input only, not as permission to delegate code changes back out.
